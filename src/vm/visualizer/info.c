...
/*   Updated: 2019/06/06 16:40:30 by aamadori         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_champions(t_vm_state *vm, t_visualizer_state *vis_state)
{
	...
	while (i < vm->players.length)
	{
		...
		mvwprintw(win.info, 14 + (i * 4), 3, "PLAYER %d :",
			((t_player*)vm->players.ptr)[i].id);
		wattron(win.info, COLOR_PAIR(2 - ((t_player*)vm->players.ptr)[i].id));
...
{
	return (state->last_alive_player);
}
...
static void	draw_end(t_game_data *game, t_visualizer_state *vis_state)
{
	...
	if (winner < game->state.players.length)
	{
		...
		mvwprintw(win.info, 32, 3, "IT'S A DRAW!");
}
