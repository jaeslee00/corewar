...
/*   Updated: 2019/06/05 22:17:02 by acolas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bigend_buffer	mem_load(t_vm_state *state, int64_t address, size_t size)
{
	...
	ft_assert(size <= REG_SIZE, "Tried to load size larger than REG_SIZE");
	index = 0;
	ret.buffer = 0;
					...
					const t_bigend_buffer store)
{
	...
	ft_assert(size <= REG_SIZE, "Tried to store size larger than REG_SIZE");
	index = 0;
