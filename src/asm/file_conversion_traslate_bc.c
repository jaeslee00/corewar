...
/*   Updated: 2019/06/05 19:09:51 by jaelee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			param_getvalue(t_list *lines, t_line *line, t_token *token)
{
	...
	else if (token->type == e_dirlab || token->type == e_indirlab)
	{
		...
		while (traverse)
		{
			...
			{
				token->value = (int)label_value(line,
						((t_line*)traverse->content), traverse);
				return (FT_SUCCESS);
			}
			traverse = traverse->next;
		}
...
{
	if (size == 1)
		bytecode[0] = (unsigned char)((unsigned int)(value) % 0x100);
	else if (size == 2)
	{
		bytecode[0] = (unsigned char)((unsigned int)(value) / 0x100);
		bytecode[1] = (unsigned char)((unsigned int)(value) % 0x100);
	}
	else if (size == 4)
	{
		bytecode[0] = (unsigned char)((unsigned int)(value)
				/ (0x100 * 0x100 * 0x100));
		bytecode[1] = (unsigned char)((((unsigned int)(value) << 2) >> 2)
				/ (0x100 * 0x100));
		bytecode[2] = (unsigned char)((((unsigned int)(value) << 4) >> 4)
				/ 0x100);
		bytecode[3] = (unsigned char)((((unsigned int)(value) << 6) >> 6)
				% 0x100);
	}
