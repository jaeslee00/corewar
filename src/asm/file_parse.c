...
/*   Updated: 2019/06/08 16:33:14 by aamadori         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			file_parse(t_file *file)
{
	...
	while (traverse)
	{
		if (~file->prework_flag & 0b10)
			set_progname(file, (t_line*)traverse->content);
		if (~file->prework_flag & 0b01)
			set_how(file, (t_line*)traverse->content);
		if (((t_line*)traverse->content)->type == e_unknown)
